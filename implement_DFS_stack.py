...
In this exercise, you'll see how to do a depth first search on a graph. 
To start, let's create a graph class in Python.
'''
...
def dfs_search(root_node, search_value):
    ...
    while len(stack) > 0:   # repeat until the stack is empty
        current_node = stack.pop()  # pop out the node added recently
        visited.add(current_node)   # markt it as visited
