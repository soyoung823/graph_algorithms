...
In the "Greedy Algorithms" lesson, 
we implemented the Dijkstra's Algorithm to find the distance of each node from the given source node. 
...
def dijkstra(graph, start_node, end_node):
    ...
    while distance_dict:
        ...
        # remove the current node from the distance_dict, 
        # and store the same key:value in shortest_distance.
        shortest_distance[current_node] = distance_dict.pop(current_node)

        # check for each neighbour of current_node, 
        # if the distance_to_neighbour is smaller than the already stored distance,
        # update the distance_dict
